defmodule RailCredo.Checks.PreferPositiveTypeGuardTest do
  use Credo.Test.Case, async: true

  alias RailCredo.Checks.PreferPositiveTypeGuard
  ...
  describe "PreferPositiveTypeGuard" do
    ...
    test "reports `not is_nil` in a boolean expression" do
      ...
      defmodule Test do
        def foo(record) do
          not is_nil(record.journal_entry) && delete(record.journal_entry)
        end
      end
      """
      |> to_source_file()
    ...
    test "does not report `not is_nil` inside a keyword-form Ecto query" do
      ...
      defmodule Test do
        ...
        def query do
          from invoice in Invoice, where: not is_nil(invoice.approved_at)
        end
      end
      """
      |> to_source_file()
    ...
    test "does not report `not is_nil` in a query when Ecto.Query is in scope via use Rail.Schema" do
      """
      defmodule Test do
        use Rail.Schema
