defmodule RailCredo.Checks.InlinePinnedFieldsInAssertTest do
  use Credo.Test.Case, async: true

  alias RailCredo.Checks.InlinePinnedFieldsInAssert
  ...
  test "flags `assert var.field == expected` after `assert {:ok, var} = call()`" do
    ...
    defmodule SomeTest do
      ...
      test "..." do
        ...
        assert fetched.vendor.id == vendor_id
      end
    end
    """
  ...
  test "flags each follow-up dot-chain assertion on a bound var" do
    ...
    defmodule SomeTest do
      ...
      test "..." do
        assert {:ok, fetched} = call()
        assert fetched.vendor.id == vendor_id
        assert fetched.gl_account.id == gl_account_id
      end
    end
    """
    |> to_source_file("test/some_test.exs")
  ...
  test "ignores patterns that already inline the shape with pins" do
    ...
    defmodule SomeTest do
      ...
      test "..." do
        assert {:ok, %{vendor: %{id: ^vendor_id}}} = call()
      end
    end
    """
    |> to_source_file("test/some_test.exs")
  ...
  test "does not treat a pinned var as a binding" do
    ...
    defmodule SomeTest do
      ...
      test "..." do
        assert {:ok, %{id: ^vendor_id}} = call()
        assert vendor_id.something == :nope
      end
    end
    """
    |> to_source_file("test/some_test.exs")
