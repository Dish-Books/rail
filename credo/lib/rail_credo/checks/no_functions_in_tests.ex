defmodule RailCredo.Checks.NoFunctionsInTests do
  @moduledoc """
  ...
  data with the most shared mechanism that fits instead of a per-file function:
    ...
    * a domain operation or fixture reused across *multiple* files (e.g.
      `user_scope/1`) -> a helper in `test/support`;
  ...
  only one test uses it, inline it; if several do, it belongs in `test/support`.

      # bad - per-file helper hides construction and can't compose across files
      defp entity_with_location(scope) do
        {:ok, entity} = Organizations.create_entity(%{scope | system: true}, %{name: "My Entity"})
        Organizations.create_location(%{scope | system: true}, %{name: "My Location", entity_id: entity.id})
        entity
      end

      # good - shared context in setup_all, construction inline
      setup_all do
        scope = user_scope(permissions: %{entities: :view})
        %{scope: scope}
      end
  ...
  use Credo.Check,
    ...
    explanations: [
      ...
      body. Move shared setup into `setup_all`/`setup`, lift a fixture reused
      across files into `test/support`, or inline whatever is distinct to a
      single test. Functions inside a nested fixture module are allowed.

          # bad
          defp build_scope, do: user_scope(permissions: %{bills: :manage})

          # good - shared across every test in the module
          setup_all do
            %{scope: user_scope(permissions: %{bills: :manage})}
          end
      """
    ]
