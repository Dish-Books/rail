defmodule RailCredo.Checks.MigrationTimestampsTest do
  use Credo.Test.Case, async: true

  alias RailCredo.Checks.MigrationTimestamps

  @migration """
  defmodule Rail.Repo.Migrations.AddLocationToBankAccounts do
    use Ecto.Migration

    def change do
      alter table(:bank_accounts) do
        add :location_id, references(:locations, type: :string)
      end
    end
  end
  """
  ...
  test "allows a generated timestamp" do
    @migration
    |> to_source_file("priv/repo/migrations/20260807135753_add_location_to_bank_accounts.exs")
  ...
  test "ignores files outside of migrations" do
    @migration
    |> to_source_file("lib/rail/banks/bank_account.ex")
    |> run_check(MigrationTimestamps)
    |> refute_issues()
  end
  ...
  test "flags a zeroed out time" do
    @migration
    |> to_source_file("priv/repo/migrations/20260806000000_add_location_to_bank_accounts.exs")
    |> run_check(MigrationTimestamps)
  ...
  test "flags a hand-written time on the hour" do
    @migration
    |> to_source_file("priv/repo/migrations/20260806120000_add_location_to_bank_accounts.exs")
    |> run_check(MigrationTimestamps)
  ...
  test "flags a hand-written time on the quarter hour" do
    @migration
    |> to_source_file("priv/repo/migrations/20260806171500_add_location_to_bank_accounts.exs")
    |> run_check(MigrationTimestamps)
    |> assert_issue(fn issue -> assert issue.message =~ "hand-written time" end)
  end
  ...
  test "flags a timestamp that is not a real date" do
    @migration
    |> to_source_file("priv/repo/migrations/20261332135753_add_location_to_bank_accounts.exs")
    |> run_check(MigrationTimestamps)
    |> assert_issue(fn issue ->
  ...
  test "flags a filename without a timestamp" do
    @migration
    |> to_source_file("priv/repo/migrations/add_location_to_bank_accounts.exs")
    |> run_check(MigrationTimestamps)
    |> assert_issue(fn issue ->
      assert issue.trigger == "add_location_to_bank_accounts.exs"
  ...
  test "flags a timestamp with too few digits" do
    @migration
    |> to_source_file("priv/repo/migrations/202608071357_add_location_to_bank_accounts.exs")
    |> run_check(MigrationTimestamps)
  ...
  test "lets migrations at or before start_after keep their hand-written time" do
    @migration
    |> to_source_file("priv/repo/migrations/20260806120000_add_location.exs")
  ...
  test "flags a hand-written time after start_after" do
    @migration
    |> to_source_file("priv/repo/migrations/20260810120000_add_location.exs")
    |> run_check(MigrationTimestamps, start_after: "20260807")
  ...
  test "flags a malformed name at or before start_after" do
    @migration
    |> to_source_file("priv/repo/migrations/20261332135753_add_location.exs")
