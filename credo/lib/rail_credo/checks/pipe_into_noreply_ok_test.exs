defmodule RailCredo.Checks.PipeIntoNoreplyOkTest do
  use Credo.Test.Case, async: true

  alias RailCredo.Checks.PipeIntoNoreplyOk
  ...
  test "allows bare {:cont, socket}" do
    """
    defmodule RailWeb.Live.Some.Hook do
      def on_mount(:default, _params, _session, socket) do
        {:cont, socket}
      end
    end
    """
    |> to_source_file("lib/rail_web/live/some/hook.ex")
    |> run_check(PipeIntoNoreplyOk)
    |> refute_issues()
  end
  ...
  test "allows 3-element tuples with same tag" do
    """
    defmodule Rail.Some.Channel do
    ...
    """
    |> to_source_file("lib/rail/some/channel.ex")
    |> run_check(PipeIntoNoreplyOk)
    |> refute_issues()
  end
