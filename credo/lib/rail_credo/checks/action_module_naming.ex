defmodule RailCredo.Checks.ActionModuleNaming do
  @moduledoc """
  ...
  1. **Public function matches module name** - `Actions.CreateBankConnection`
     must export `create_bank_connection/N`. Functions with `?` or `!` suffixes
     are accepted (e.g., `can?`, `upsert_order_category!`).

  2. **Filename matches module name** - `Actions.CreateBankConnection` must
     live in `create_bank_connection.ex`.
  ...
  use Credo.Check,
    ...
    explanations: [
      ...
      exported public function, and filename all align.
          ...
          # File: lib/rail/banks/actions/create_bank_connection.ex
