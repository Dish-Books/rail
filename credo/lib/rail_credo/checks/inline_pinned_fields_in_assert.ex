defmodule RailCredo.Checks.InlinePinnedFieldsInAssert do
  @moduledoc """
  ...
  preceding `assert <pattern with var> = call()` using a pinned pattern.
      ...
      # bad
      assert {:ok, item} = Inventory.get_item(scope, id: id)
      assert item.vendor.id == vendor.id
      assert item.gl_account.id == gl_account.id

      # good
      %{id: vendor_id} = vendor
      %{id: gl_account_id} = gl_account

      assert {:ok, %{vendor: %{id: ^vendor_id}, gl_account: %{id: ^gl_account_id}}} =
               Inventory.get_item(scope, id: id)
  ...
  `item.vendor.id`) or compared directly via `==`/`===` (e.g.
  `assert id == client.id`). The fix is to fold the expected field into the
  ...
  use Credo.Check,
    ...
    explanations: [
      ...
      var and reading fields off it in follow-up assertions.
          ...
          # good
          assert {:ok, %{id: ^expected_id, name: "X"}} = call()
      """
    ]
