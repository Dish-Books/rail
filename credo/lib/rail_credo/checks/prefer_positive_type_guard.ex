defmodule RailCredo.Checks.PreferPositiveTypeGuard do
  @moduledoc """
  ...
  defp collect_ecto_is_nil_metas(ast) do
    ...
    {_ast, set} =
      Macro.prewalk(ast, MapSet.new(), fn
        ...
        # happens to share a name (e.g. RailWeb.Utils.Join.join/3) is not exempted.
  ...
  @ecto_query_scope_uses [[:Rail, :Schema], [:Rail, :DataCase]]

  # Detects whether `Ecto.Query`'s macros are in scope - either imported directly
  # or pulled in via a `use` that imports it (e.g. `use Rail.Schema`).
