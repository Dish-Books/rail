...
export async function openQaSession({ port, debugPort, scratchDir }) {
  ...
  // Headless Chrome on Linux renders WebGL in software and says so, loudly, on every page with a
  // chart - which is most of them. Matched against the browser's own log text only: a console error,
  // an exception or a failed request keeps its message whatever words happen to be in it.
  const rendererNoise = /Automatic fallback to software WebGL|GPU stall|swiftshader|GroupMarkerNotSet/i
  ...
  session.on('Log.entryAdded', ({ entry }) => {
    ...
    if (rendererNoise.test(entry.text)) return
