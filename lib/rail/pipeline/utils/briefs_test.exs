defmodule Rail.Pipeline.Utils.BriefsTest do
  ...
  describe "review_brief/1" do
    test "carries only the verdict contract" do
      brief = review_brief(scratch_path: @scratch)
    ...
    test "includes branch and git diff when branch is provided" do
      brief = review_brief(scratch_path: @scratch, branch: "feature/login")

      assert brief =~ "The change is on branch feature/login; read it with `git diff main...HEAD` from your worktree."

      custom_base = review_brief(scratch_path: @scratch, worktree_name: "feature/pay", base_branch: "develop")
  ...
  describe "qa_brief/1" do
    test "instructs driving the running app and writing evidence to scratch" do
      brief = qa_brief(scratch_path: @scratch)

      assert brief =~ "Write your evidence to #{@scratch}/qa/ with a manifest.json"
    ...
    test "includes branch line when branch is provided" do
      brief = qa_brief(scratch_path: @scratch, branch_name: "feature/checkout")
  ...
  describe "qa_lead_brief/1" do
    test "instructs grading the QA pass with evidence in scratch" do
      brief = qa_lead_brief(scratch_path: @scratch)

      assert brief =~ "The QA engineer's report is above and its evidence is in #{@scratch}/qa/"
      assert brief =~ "`VERDICT: PASS` or `VERDICT: FAIL`"
    ...
    test "includes branch line when branch is provided" do
      brief = qa_lead_brief(scratch_path: @scratch, branch: "fix/nav")
  ...
  describe "demo_brief/1" do
    test "includes fallback when no criteria are found" do
      brief = demo_brief(scratch_path: @scratch)
      ...
      assert brief =~ "Rail reads the recording from #{@scratch}/demo/."
      assert brief =~ ~s("path": "#{@scratch}/demo/<frame>.png")
      refute brief =~ ".rail/demo"
    end

    test "formats criteria list numbered in order" do
      brief = demo_brief(scratch_path: @scratch, criteria: ["User clicks login", "Error toast appears on invalid email"])
    ...
    test "extracts acceptance criteria from ticket description markdown" do
      ...
      brief = demo_brief(scratch_path: @scratch, ticket: ticket)
  ...
  describe "demo_rerecord_brief/1" do
    test "matches golden output when comment is provided" do
      brief = demo_rerecord_brief("First attempt missed the validation modal", scratch_path: @scratch)
    ...
    test "matches golden output without comment" do
      brief = demo_rerecord_brief(scratch_path: @scratch, version: 3)
  ...
  describe "rebase_brief/1" do
    test "formats default rebase brief" do
      brief = rebase_brief(scratch_path: @scratch)
    ...
    test "formats rebase brief with custom branch, base, and pr number" do
      brief = rebase_brief(scratch_path: @scratch, branch: "fix-auth", base_branch: "develop", pr_number: 142)
  ...
  describe "stage_brief/2 top-level dispatcher" do
    test "returns empty string for terminal stages, unknown stages, or nil" do
      ...
      assert stage_brief(:ready_to_merge, scratch_path: @scratch) == ""
      assert stage_brief(:merged, scratch_path: @scratch) == ""
      assert stage_brief(:unknown_stage, scratch_path: @scratch) == ""
    ...
    test "dispatches based on stage atom" do
      ...
      assert stage_brief(:demo, scratch_path: @scratch) =~ "#{@scratch}/demo/manifest.json"
    end
    ...
    test "dispatches based on stage string" do
      ...
      assert stage_brief("ready_to_merge", scratch_path: @scratch) == ""
      assert stage_brief("merged", scratch_path: @scratch) == ""
      assert stage_brief("other", scratch_path: @scratch) == ""
    end
    ...
    test "handles edge cases and non-map/struct parameters cleanly" do
      assert demo_rerecord_brief(scratch_path: @scratch) =~ "Re-record the demo."
