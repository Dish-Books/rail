defmodule Rail.Pipeline.Actions.StartQaRun do
  ...
  defp brief(%Task{scratch_path: scratch_path, issue: %Issue{} = issue} = task) do
    ...
    The browser is Rail's rather than yours. `browser_goto`, `browser_do`, `browser_look`, `qa_shot` and `browser_problems` drive one headless Chrome that stays where you left it between calls; it opens on the first call and Rail closes it when the task moves on, so there is nothing to start and nothing to stop.

    Drive by outcome. `browser_do` takes what you want to be true of the page - "a bill for Sysco dated 12 Aug 2026 for $2,500 is entered and saved" - with every value it needs in `values`, keyed by the field's label, and it loops until it gets there. A keystroke at a time is slower and worse: each call begins knowing nothing of the last, so the same step gets chosen twice. Then read the page with `browser_look` before you mark the check: reaching the end of an instruction is not the same as the application having done the right thing, and the check is about the second.
