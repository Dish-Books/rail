defmodule Rail.Pipeline.Actions.DetectQuestionTest do
  ...
  test "detects a question on its own line" do
    question = Pipeline.detect_question("[QUESTION: Which database should we use?]")
  ...
  test "detects question with options" do
    ...
    question = Pipeline.detect_question(line)
  ...
  test "allows leading whitespace, blockquotes, and bullet markers" do
    assert %DetectedQuestion{prompt: "Bullet question"} =
             Pipeline.detect_question("- [QUESTION: Bullet question]")

    assert %DetectedQuestion{prompt: "Star bullet question"} =
             Pipeline.detect_question("* [QUESTION: Star bullet question]")

    assert %DetectedQuestion{prompt: "Blockquote question"} =
             Pipeline.detect_question("> [QUESTION: Blockquote question]")

    assert %DetectedQuestion{prompt: "Nested prefix question"} =
             Pipeline.detect_question("  > * -  [QUESTION: Nested prefix question]")
  end

  test "ignores question marker quoted mid-sentence in prose" do
    assert is_nil(Pipeline.detect_question("The user asked [QUESTION: what about this?] earlier in the discussion."))
  end

  test "rejects placeholder prompts echoing role brief templates" do
    assert is_nil(Pipeline.detect_question("[QUESTION: <question>]"))
    assert is_nil(Pipeline.detect_question("[QUESTION: <describe your question here>]"))
    assert is_nil(Pipeline.detect_question("[QUESTION: ...]"))
    assert is_nil(Pipeline.detect_question("[QUESTION: …]"))
    assert is_nil(Pipeline.detect_question("[QUESTION:   . . .   ]"))
    assert is_nil(Pipeline.detect_question("[QUESTION: ]"))
    assert is_nil(Pipeline.detect_question("[QUESTION:    ]"))
  ...
  test "handles nil and empty strings" do
    assert is_nil(Pipeline.detect_question(nil))
    assert is_nil(Pipeline.detect_question(""))
    assert is_nil(Pipeline.detect_question("   "))
  end
  ...
  test "finds question in multi-line text" do
    ...
    * [QUESTION: Should we use Postgres or SQLite?] [OPTIONS: Postgres, SQLite]
    ...
    question = Pipeline.detect_question(text)
    assert %DetectedQuestion{} = question
  ...
  test "extracts task_id, role_id, id, and context_summary from options map" do
    ...
    question = Pipeline.detect_question("[QUESTION: What port?]", opts)
  ...
  test "extracts task and role details from structs/maps in keyword opts" do
    ...
    question = Pipeline.detect_question("[QUESTION: Which OAuth provider?]", opts)
  ...
  test "extracts context_summary from task_title option" do
    ...
    question = Pipeline.detect_question("[QUESTION: Refund amount?]", opts)
  ...
  test "extracts task and role from atom map access" do
    ...
    question =
      Pipeline.detect_question("[QUESTION: Query?]", %{task: task, role: role})
  ...
  test "detect_questions returns every question in order, collapsing repeats" do
    ...
    questions = Pipeline.detect_questions(text, task_id: "tsk_1", role_id: "rol_1")
  ...
  test "detect_questions returns an empty list when there is nothing to ask" do
    assert Pipeline.detect_questions("Just prose.") == []
    assert Pipeline.detect_questions(nil) == []
  end

  test "detect_question still returns only the first question" do
    text = "[QUESTION: First?]\n[QUESTION: Second?]"

    assert %DetectedQuestion{prompt: "First?"} = Pipeline.detect_question(text)
  end
end
