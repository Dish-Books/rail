defmodule Rail.Pipeline.Actions.RunFinishedTest do
  ...
  test "a run that stated its verdict latches done and enters the next stage", %{
    task: task,
    roles: roles,
    exited: exited
  } do
    ...
    Runs.append_run_event(run, "All done.\n\nVERDICT: DONE")

    assert {:ok, %Run{stage_outcome: :done}} = Pipeline.run_finished(os_process, %{exit_code: 0})

    assert %Task{stage: :review} = Repo.reload!(task)
    assert Repo.get_by(Run, task_id: task.id, role_id: roles[:review].id)
  end

  test "a run that said nothing is left alone for the next message to pick up", %{
    task: task,
    exited: exited
  } do
    {:ok, task} = Pipeline.update_task(task, %{stage: :engineer})
    ...
    Runs.append_run_event(run, "I got half way through.")
  ...
  test "a run that already had its say is left alone however often it exits", %{
    task: task,
    exited: exited
  } do
    {:ok, task} = Pipeline.update_task(task, %{stage: :engineer})
    {run, os_process} = exited.(:engineer, %{stage_outcome: :done})
  ...
  test "a rebase settles on its own terms, whatever the engineer concluded before", %{
    task: task,
    exited: exited
  } do
  ...
  test "each stage's finish is its own run's, never the task's", %{task: task, roles: roles, exited: exited} do
    # A message to the reviewer settles the review, whatever the task moved on to.
    {:ok, task} = Pipeline.update_task(task, %{stage: :qa_lead, worktree_path: create_temp_git_repo()})
    ...
    # The review passed, so the change goes to QA — not wherever the task sat.
    assert %Task{stage: :qa} = Repo.reload!(task)
    assert Repo.get_by(Run, task_id: task.id, role_id: roles[:qa].id)
  end
