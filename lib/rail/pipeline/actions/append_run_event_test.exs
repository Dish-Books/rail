defmodule Rail.Pipeline.Actions.AppendRunEventTest do
  ...
  test "takes a run struct or an id and keeps the log in order", %{run: run} do
    assert %RunEvent{line: "from struct", seq: first} = Pipeline.append_run_event(run, "from struct")
    assert %RunEvent{line: "from id", seq: second} = Pipeline.append_run_event(run, "from id")
  ...
  test "leaves the line on the run rather than any one of its processes", %{run: run} do
    assert %RunEvent{os_process_id: nil} = Pipeline.append_run_event(run, "[rail] between turns")
  end

  test "broadcasts the event on the run topic", %{run: %Run{id: run_id} = run} do
    Phoenix.PubSub.subscribe(Rail.PubSub, "run:#{run_id}")

    event = Pipeline.append_run_event(run, "broadcast me")
