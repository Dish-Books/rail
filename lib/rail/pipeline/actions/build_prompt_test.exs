defmodule Rail.Pipeline.Actions.BuildPromptTest do
  ...
  test "emits role instructions inside tags for Agy on the first run only" do
    ...
    first_prompt = Pipeline.build_prompt(first_opts)
    ...
    resume_prompt = Pipeline.build_prompt(resume_opts)
  ...
  test "handles empty options and non-standard backends" do
    ...
    assert Pipeline.build_prompt(backend: nil, task_description: "Hello") == "Hello\n"
    assert Pipeline.build_prompt(backend: 123, task_description: "Hello") == "Hello\n"
  end
end
