defmodule Rail.Mcp.Utils.RunToolQaGoto do
  @moduledoc """
  Opens a URL in the QA browser and says where it ended up.

  Where it ended up is not always where it was sent - a login redirect, a
  canonical path, a route that bounced - and a pass that assumed otherwise would
  check the wrong screen. The page is read once it has settled rather than the
  instant navigation returns, because a document mid-render has no title yet.
  """
  ...
  """
  def run_tool_qa_goto(%Task{} = task, %{"url" => url}, opts) do
