defmodule Rail.Runs.FollowerSupervisor do
  ...
  and the backend whose stream format it speaks -- so `opts` carries only what the
  row cannot: the `:port` of a freshly spawned child, `:skip_log_lines` for a stream
  already partly recorded, and the tick intervals.
