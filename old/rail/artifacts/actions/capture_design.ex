defmodule Rail.Artifacts.Actions.CaptureDesign do
  ...
  def capture_design(_scope, target, scratch_dir_or_opts, opts \\ []) do
    {task_id, scratch_dir, combined_opts} = normalize_args(target, scratch_dir_or_opts, opts)
    do_capture_design(task_id, scratch_dir, combined_opts)
  end
  ...
  defp do_capture_design(task_id, scratch_dir, opts) do
    design_dir = resolve_design_dir(scratch_dir)

    with {:ok, design_data} <- DesignValidator.validate(design_dir, opts),
         {:ok, directions_with_assets} <- upload_design_assets(design_data.directions, opts) do
      ...
      with {:ok, comment_id} <- maybe_post_comment(design_struct, opts) do
        final_attrs =
          if comment_id do
  ...
  defp upload_design_assets(directions, opts) do
    directions
    |> Enum.reduce_while({:ok, []}, fn dir_entry, {:ok, acc} ->
      case upload_direction_still(dir_entry, opts) do
  ...
  defp upload_direction_still(dir_entry, opts) do
  ...
  defp maybe_post_comment(design_struct, opts) do
